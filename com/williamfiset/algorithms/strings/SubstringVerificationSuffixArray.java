/**
 * This file shows you how to use a suffix array to determine if
 * a pattern exists within a text. This implementation has the advantage
 * that once the suffix array is built queries can be very fast.
 *
 * Time complexity: O(nlogn) for suffix array construction and O(mlogn)
 * time for individual queries (where m is query string length). 
 * As noted below, depending on the length of the string (if it is very large) 
 * it may be faster to use KMP or if you're doing a lot of queries on small
