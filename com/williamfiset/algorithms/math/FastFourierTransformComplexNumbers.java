/** 
 * This snippet multiplies 2 complex polynomials very efficiently 
...
public class FastFourierTransformComplexNumbers {
  ...
  static Complex[] pad(Complex[] arr, int n) {
    ...
    for (int i = 0; i < n; i++)
      padded[i] = (i < arr.length) ? arr[i] : new Complex(0, 0);
    return padded;
  }
  ...
  static Complex[] fft(Complex[] x) {
    ...
    Complex[] r = fft(arr); Complex[] y = new Complex[n];
    for (int k = 0; k < n/2; k++) {
  ...
  static Complex[] ifft(Complex[] x) {
    int n = x.length; Complex[] y = new Complex[n];
